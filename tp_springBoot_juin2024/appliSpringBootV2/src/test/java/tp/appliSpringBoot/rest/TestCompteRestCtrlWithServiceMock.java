...
public class TestCompteRestCtrlWithServiceMock {
	...
	private MockMvc mvc;
	@MockBean
	private CompteRepository compteRepository; // not real implementation but mock to configure .

	@BeforeEach
	public void reInitMock() {
		//vérification que le service injecté est bien un mock
		assertTrue(Mockito.mockingDetails(compteRepository).isMock());
		//reinitialisation du mock(de scope=Singleton par defaut) sur aspects stub et spy
		Mockito.reset(compteRepository);
	}
	...
	public void testComptesDuClient1WithMockOfCompteService() {
		...
		comptes.add(new Compte(1L, "compteC", 250.0));
		comptes.add(new Compte(2L, "compteD", 350.0));
		Mockito.when(compteRepository.findBySoldeGreaterThanEqual(200)).thenReturn(comptes);
		try {
			MvcResult mvcResult = mvc
					.perform(get("/rest/api-bank/compte?soldeMini=200").contentType(MediaType.APPLICATION_JSON))
					.andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(2)))
					.andExpect(jsonPath("$[0].label", is("compteC"))).andExpect(jsonPath("$[1].solde", is(350.0)))
					.andReturn();
